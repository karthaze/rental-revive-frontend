import{e as k,g as C,a as $}from"./backend-DfcKIe5O.js";import{m as y}from"./onboard-BS00hYcv.js";import"./places-D2Zs4JB8.js";const i=e=>String(e??"").replace(/[&<>"]/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"})[t]),A={phone:{label:"Phone probe",icon:'<svg viewBox="0 0 24 24"><path d="M22 16.9v3a2 2 0 0 1-2.2 2 19.8 19.8 0 0 1-8.6-3.1 19.5 19.5 0 0 1-6-6A19.8 19.8 0 0 1 2.1 4.2 2 2 0 0 1 4.1 2h3a2 2 0 0 1 2 1.7c.1 1 .4 2 .7 2.8a2 2 0 0 1-.5 2.1L8.1 9.9a16 16 0 0 0 6 6l1.3-1.3a2 2 0 0 1 2.1-.4c.9.3 1.9.5 2.8.7a2 2 0 0 1 1.7 2z"/></svg>'},email:{label:"Email inquiry",icon:'<svg viewBox="0 0 24 24"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-10 6L2 7"/></svg>'},form:{label:"Website form",icon:'<svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M7 8h10M7 12h6"/><path d="M7 16h4"/></svg>'}},w={business:"business hours",lunch:"lunch window",after_hours:"after close",alt_day:"a different day"},x=e=>{if(e<6e4)return`${Math.max(1,Math.round(e/1e3))} seconds`;if(e<36e5)return`${Math.round(e/6e4)} minutes`;const t=e/36e5;return t>=48?`${Math.round(t/24)} days`:`${Math.round(t*10)/10} hours`},g=(e,t)=>{try{return new Intl.DateTimeFormat("en-US",{timeZone:t,weekday:"short",hour:"numeric",minute:"2-digit"}).format(e)}catch{return new Date(e).toLocaleString()}};function M(e,t,n){const s=e.metrics||{};if(e.outcome===null){if(e.dispatchedAt===null)return e.scheduledFor>n?{cls:"wait",text:`Scheduled · ${g(e.scheduledFor,t)}${e.window?` — ${w[e.window]}`:""}`}:{cls:"wait",text:"Queued…"};if(e.channel==="phone"){if(s.answeredAt)return{cls:"live",text:"Connected — on the line now"};if(s.ringStartedAt){const o=Math.max(1,Math.round((n-s.ringStartedAt)/6e3));return{cls:"live",text:`Ringing… about ${o} ring${o>1?"s":""}`}}return{cls:"live",text:"Dialling…"}}return{cls:"wait",text:s.deliveryStatus==="delivered"||s.submissionSucceeded?"Delivered · awaiting a reply — the clock is running":"Sent · confirming delivery"}}switch(e.outcome){case"responded":{if(e.channel==="phone"){const r=s.estimatedRings;return{cls:"ok",text:`Answered${r?` in about ${r} ring${r>1?"s":""}`:""}`}}const o=typeof s.msToFirstReply=="number"?` in ${x(s.msToFirstReply)}`:"",a=[s.containedPrice?"included a price":null,s.containedNextStep?"asked a next step":null].filter(Boolean).join(", ");return{cls:"ok",text:`Human reply${o}${a?` — ${a}`:""}`}}case"no_response":return e.channel==="phone"?s.answeredBy==="voicemail"?{cls:"gap",text:`Voicemail${s.voicemailBoxFull?" — box full":""} — audit message left`}:{cls:"gap",text:"Rang out — nobody answered"}:{cls:"gap",text:"Delivered — no reply inside the window"};case"undeliverable_theirs":return e.channel==="phone"?{cls:"gap",text:"Number invalid or disconnected"}:e.channel==="email"?{cls:"gap",text:"Published address bounced"}:{cls:"gap",text:s.submissionSucceeded===!1&&s.fieldsFilled?"Form broke — it ate the submission":"No working inquiry form found"};case"blocked_by_target":return{cls:"gap",text:"Blocked by a captcha on your form — real customers hit it too"};case"undeliverable_ours":return{cls:"ours",text:"We couldn’t get through — our side, not yours. Not counted against you."};default:return{cls:"wait",text:"Cancelled"}}}function S(e,t,n){const s=A[e.channel],o=M(e,t,n),a=e.dispatchedAt??e.scheduledFor;return`
    <div class="pd-row ${o.cls}">
      <span class="pd-ic">${s.icon}</span>
      <span class="pd-txt">
        <b>${s.label}${e.channel==="phone"?` · attempt ${e.sequence}`:""}</b>
        <small>${o.text}</small>
      </span>
      <span class="pd-when">${g(a,t)}</span>
    </div>`}function T(e,t,n){const s=e.find(a=>a.channel==="phone"&&a.outcome===null&&a.dispatchedAt!==null),o=e.find(a=>a.channel==="phone"&&a.outcome===null&&a.dispatchedAt===null&&a.scheduledFor>n);if(!s&&!o)return"";if(s){const a=s.metrics||{},r=a.answeredAt?"CONNECTED":a.ringStartedAt?"RINGING":"DIALLING",l=a.ringStartedAt&&!a.answeredAt?Math.max(1,Math.round((n-a.ringStartedAt)/6e3)):null;return`
    ...
    <div class="pd-live" data-phase="scheduled">
      ...
      <div class="pd-live-txt">
        <b>NEXT CALL · ${i(g(o.scheduledFor,t).toUpperCase())}</b>
        <small>${i(w[o.window]||"scheduled")} — attempts are spread across the day on purpose.</small>
      </div>
    </div>`}function N(e){const t=e.verdict;if(!t)return"";const n=t.counts,s=t.measured&&t.measured.substitutions||[],o=[`${n.dispatched} ${n.dispatched===1?"inquiry":"inquiries"} went out.`,n.reachedHuman?`${n.reachedHuman} reached a human.`:"None reached a human.",n.noResponse?`${n.noResponse} never got a response.`:null,n.unreachableOurs?`${n.unreachableOurs} couldn’t be delivered by our side — not counted.`:null,t.fastestResponseMs!==null?`Fastest response: ${x(t.fastestResponseMs)}.`:null].filter(Boolean).join(" ");let a="";if(t.repriced&&s.length){const r=t.repriced.monthlyCents/100,l=e.estimate?e.estimate.monthlyCents/100:null;a=`
    ...
    </div>`}function _(e,t={}){var r;const n=t.now??Date.now(),s=e.run.timezone,o=document.createElement("div");o.className="probe-dash";const a=e.run.status==="active"?e.attempts.some(l=>l.outcome===null&&l.dispatchedAt!==null&&l.channel==="phone")?"LIVE":"IN FLIGHT":e.run.status==="resolved"?"VERDICT":e.run.status==="killed"?"STOPPED":"EXPIRED";return o.innerHTML=`
    <div class="pd-head">
      <span class="pd-title">
        <b>Proof load test</b>
        <small>${i(e.yardName||"your yard")} · all times ${i(s)}</small>
      </span>
      <span class="pd-status s-${a.toLowerCase().replace(" ","")}">${a}</span>
      ${e.run.status==="active"?'<button class="pd-kill" type="button" data-kill>Stop everything</button>':""}
    </div>
    ${e.run.status==="active"?T(e.attempts,s,n):""}
    ...
    ${e.map(n=>n.expired?`<span class="pd-proof expired">${i(h(n.kind))} — expired per the retention window you were shown</span>`:n.contentType.startsWith("audio/")?`<figure class="pd-proof"><figcaption>${i(h(n.kind))}</figcaption><audio controls preload="none" src="${i(n.url)}"></audio></figure>`:n.contentType.startsWith("image/")?`<figure class="pd-proof"><figcaption>${i(h(n.kind))}</figcaption><img loading="lazy" src="${i(n.url)}" alt="${i(h(n.kind))}"></figure>`:`<a class="pd-proof" href="${i(n.url)}" target="_blank" rel="noopener">${i(h(n.kind))}</a>`).join("")}`,t}const h=e=>({call_recording:"Call recording",voicemail_recording:"The voicemail we left in your box",call_transcript:"Call transcript",email_body:"The inquiry email",form_screenshot_before:"Your form, filled in",form_screenshot_after:"Your form, after submitting"})[e]||e;async function R(e,t){const s=await k()?await C():null,o=document.createElement("div");if(o.className="probe-dash-shell",e.appendChild(o),!s)return o.innerHTML=`<div class="probe-dash"><p class="pd-note">
      ...
      the link in your report email.</p></div>`,()=>o.remove();let a=null,r=null;const l=new Map,d=()=>{var u;if(!a)return;const c=_(a,{now:Date.now(),onKill:async()=>{confirm("Stop the test? Every scheduled probe stands down immediately.")&&await s.mutation($.runs.kill.killRun,{runId:t})}}),m=c.querySelector("[data-artifacts]");for(const p of a.attempts){if(!((u=p.artifactIds)!=null&&u.length))continue;const f=l.get(p.id);if(f!=null&&f.length){const v=E(f);v&&m.appendChild(v)}else l.has(p.id)||(l.set(p.id,[]),s.query($.runs.queries.attemptArtifacts,{attemptId:p.id}).then(v=>{l.set(p.id,v||[]),d()}))}o.replaceChildren(c)},b=s.onUpdate($.runs.queries.runState,{runId:t},c=>{if(a=c,!c){o.innerHTML=`<div class="probe-dash"><p class="pd-note">
        This run isn’t visible from this account. Runs are only shown to
        the owner who authorised them.</p></div>`;return}d();const m=c.run.status==="active"&&c.attempts.some(u=>u.channel==="phone"&&u.outcome===null&&u.dispatchedAt!==null);m&&!r&&(r=setInterval(d,1e3)),!m&&r&&(clearInterval(r),r=null)});return()=>{b==null||b(),r&&clearInterval(r),o.remove()}}export{R as mountDashboard};
