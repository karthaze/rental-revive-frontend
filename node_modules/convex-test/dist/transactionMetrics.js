...
export class TransactionMetricsTracker {
    ...
    startTransaction(transactionLimits) {
        const parent = this._current();
        this._layers.push(parent === undefined
            ? this._rootLayer()
    ...
    trackRead(docSizeBytes) {
        ...
        if (layer.enforce) {
            if (layer.metrics.bytesRead > layer.limits.bytesRead) {
                throw new Error(`Read too much data in a single function execution (limit: ${layer.limits.bytesRead} bytes). ` +
            ...
            if (layer.metrics.documentsRead > layer.limits.documentsRead) {
                throw new Error(`Scanned too many documents in a single function execution (limit: ${layer.limits.documentsRead}). ` +
                    `This is a Convex limit: https://docs.convex.dev/production/state/limits`);
            }
        }
    }
    trackWrite(docSizeBytes) {
        ...
        if (layer.enforce) {
            if (layer.metrics.bytesWritten > layer.limits.bytesWritten) {
                throw new Error(`Wrote too much data in a single function execution (limit: ${layer.limits.bytesWritten} bytes). ` +
                    `This is a Convex limit: https://docs.convex.dev/production/state/limits`);
            }
            if (layer.metrics.documentsWritten > layer.limits.documentsWritten) {
                throw new Error(`Wrote too many documents in a single function execution (limit: ${layer.limits.documentsWritten}). ` +
                    `This is a Convex limit: https://docs.convex.dev/production/state/limits`);
            }
        }
    }
    trackIndexRange() {
        ...
        if (layer.enforce) {
            if (layer.metrics.databaseQueries > layer.limits.databaseQueries) {
                throw new Error(`Too many index ranges read in a single function execution (limit: ${layer.limits.databaseQueries}). ` +
                    `This is a Convex limit: https://docs.convex.dev/production/state/limits`);
            }
        }
    }
    trackScheduledFunction(argsSizeBytes) {
        ...
        if (layer.enforce) {
            if (layer.metrics.functionsScheduled > layer.limits.functionsScheduled) {
                throw new Error(`Scheduled too many functions in a single function execution (limit: ${layer.limits.functionsScheduled}). ` +
                    `This is a Convex limit: https://docs.convex.dev/production/state/limits`);
            }
            if (layer.metrics.scheduledFunctionArgsBytes >
                layer.limits.scheduledFunctionArgsBytes) {
                throw new Error(`Scheduled function arguments too large in a single function execution (limit: ${layer.limits.scheduledFunctionArgsBytes} bytes). ` +
                    `This is a Convex limit: https://docs.convex.dev/production/state/limits`);
            }
        }
    }
    getTransactionMetrics() {
        ...
        return {
            ...
            scheduledFunctionArgsBytes: {
                ...
                remaining: limits.scheduledFunctionArgsBytes -
    ...
    _childLayer(parent, transactionLimits) {
        ...
        if (transactionLimits !== undefined) {
            for (const key of Object.keys(limits)) {
                ...
                if (requested !== undefined) {
                    ...
                    limits[key] = Math.min(parent.limits[key], parent.metrics[key] + requested);
                }
            }
        }
        return {
